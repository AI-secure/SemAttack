        ...
        if self.mask is None:
            ...
            for i in range(input_adv.size(1)):
                ...
                for j in range(add_start, add_end):
                    similar_wv = model.encoder.bilstm.encoder(torch.LongTensor(self.wv[self.batch_info['data'][j][i].item()]).cuda())
                    ...
                    input_adv.data[j, i] = self.itereated_var.data[j, i] = similar_wv[new_word.item()].data
                    del temp_place
