        ...
        if self.mask is None:
            ...
            for i in range(input_adv.size(0)):
                ...
                for j in range(add_start, add_end):
                    ...
                    if self.word_mode:
                        if self.batch_info['start_mark'][j] == 1:
                            if j == len(self.batch_info['start_mark']) - 1:
                                k = j + 1
                            else:
                                k = self.batch_info['start_mark'].index(1, j+1)
                            similar_wv = model.bert.embeddings.word_embeddings(
                                torch.LongTensor(self.wv[tuple(self.seq[i][j:k].cpu().numpy())]).cuda())
                            ...
                            input_adv.data[i, j:k] = self.itereated_var.data[i, j:k] = similar_wv[new_word.item()].data.view(-1, input_adv.shape[2])
                            del temp_place
                    else:
